...
class TripletsDataset(BaseDataset):
    ...
    def compute_cache(args, model, subset_ds, cache_shape):
        ...
        cache_a_shape = [cache_shape[0],384,24,24]
        cache_a = RAMEfficient4DMatrix(cache_a_shape, dtype=np.float32)
        with torch.no_grad():
            for images, indexes in tqdm(subset_dl, ncols=100):              
                images = images.to(args.device)
                features, features_a = model("features_extractor", [images, "global"])
                cache[indexes.numpy()] = features.cpu().numpy()
                cache_a[indexes.numpy()] = features_a.cpu().numpy()
        return cache, cache_a
    ...
    def get_best_positive_index(self, args, model, query_index, cache, cache_a, query_features):
    ...
    def compute_triplets_full(self, args, model):
        ...
        cache, cache_a = self.compute_cache(args, model, subset_ds, (len(self), args.features_dim))
        ...
        for query_index in tqdm(sampled_queries_indexes, ncols=100):
            ...
            best_positive_index = self.get_best_positive_index(args, model, query_index, cache, cache_a, query_features)
    ...
    def compute_triplets_partial(self, args, model):
        ...
        subset_ds = Subset(self, database_indexes + list(sampled_queries_indexes + self.database_num))
        cache, cache_a = self.compute_cache(args, model, subset_ds, cache_shape=(len(self), args.features_dim))
